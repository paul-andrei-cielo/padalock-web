    ...
    <main className="h-screen overflow-hidden bg-gradient-to-b from-[#df4473] via-[#e99ab1] to-[#f4eff1] px-4 py-4 md:px-6 md:py-5 lg:px-8 lg:py-6">
      <div className="mx-auto flex h-full w-full flex-col gap-4">
        <header className="relative z-50 shrink-0 rounded-[1.5rem] bg-[#FFFFFF]/25 px-4 py-3 backdrop-blur-sm md:px-6 md:py-3 lg:px-8 lg:py-4">
          ...
          {isMenuOpen && (
            <nav className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-50 flex flex-col gap-y-1.5 rounded-[1.5rem] bg-white/85 p-4 text-center text-sm font-bold text-[#df4473] shadow-xl backdrop-blur-xl border border-white/30 transition-all lg:hidden">
              {navItems.map((item) => (
                <Link
                  key={item.label}
                  href={item.href}
                  onClick={() => setIsMenuOpen(false)}
                  className="rounded-xl py-2.5 transition hover:bg-[#df4473]/10 active:bg-[#df4473]/20"
                >
                  {item.label}
                </Link>
              ))}
